...
def run_unsupervised_training(sess, load_params_path = None, save1=None, save2=None, save3=None):
    ...
    batch_num = 1
    total_correct = 0
    top_beam = open("top beam result train after uns prunning.txt ", 'w')
    ...
    while train.epochs_completed < 1:
        ...
        for step in range (len(sentences)):
            ...
            beam = e_greedy_randomized_beam_search(next_token_probs_getter, logical_tokens_mapping,
                                                   original_sentence=sentence,  epsilon = 0)
        ...
        #if save_model_path and train.epochs_completed % 4 == 3:
        #    saver2.save(sess, save_model_path, global_step=4,write_meta_graph=False)
        #prev_epoch = train.epochs_completed
        #   print("saved epoch %d" % train.epochs_completed)
    if save1:
        saver1.save(sess, save1)
    if save2:
        saver2.save(sess, save2)
    if save3:
        saver3.save(sess, save3)
...
def run_supervised_training(sess, load_params_path=None, save1=None, save2=None, save3=None):
    # build the computaional graph:
    # bi-lstm encoder - given a sentence (of a variable length) as a sequence of word embeddings,
    # and returns the lstm outputs.
    sentence_placeholder, sent_lengths_placeholder, h, e_m = build_sentence_encoder()
...
TRAINED_WEIGHTS_SUP1 = os.path.join(definitions.ROOT_DIR, 'seq2seqModel' ,'learnedWeights','trained_variables_sup1.ckpt')
TRAINED_WEIGHTS_SUP2 = os.path.join(definitions.ROOT_DIR, 'seq2seqModel' ,'learnedWeights','trained_variables_sup2.ckpt')
TRAINED_WEIGHTS_SUP3 = os.path.join(definitions.ROOT_DIR, 'seq2seqModel' ,'learnedWeights','trained_variables_sup3.ckpt')
...
if __name__ == '__main__':
    with tf.Session() as sess:
        ...
        #run_supervised_training(sess,save1=TRAINED_WEIGHTS_SUP1,save2=TRAINED_WEIGHTS_SUP2,save3=TRAINED_WEIGHTS_SUP3)
        ### !!! remember to change saver type inside function
        run_supervised_training(sess, load_params_path=TRAINED_WEIGHTS_SUP3)
        #start = time.time()
        ### !!!remember to change epsilon and remove gradient comment!!!
